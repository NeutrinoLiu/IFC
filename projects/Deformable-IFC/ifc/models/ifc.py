...
class IFC(nn.Module):
    ...
    def __init__(self, backbone, transformer, mask_head, num_classes, num_queries, num_frames, aux_loss=False):
        ...
        self.input_proj_list = nn.ModuleList([nn.Conv2d(channel_size, hidden_dim, kernel_size=1) for channel_size in backbone.num_channels])
    ...
    def forward_post_backbone(self, features, pos, is_train=True):
        # cs839 for deformable-IFC we need all layer features from backbone instead of the last layer output
        raw_list = []
        mask_list = []
        for layer in features:
            src, mask = layer.decompose()
            raw_list.append(src)
            mask_list.append(mask)
        src_proj = [projector(src) for projector, src in zip(self.input_proj_list, raw_list)]
        # now you have 4 layers of 256-channel feature maps 4 x NT x HW x 256

        clip_hs, encoded_src = self.transformer(   # LxQxBxTxC
            src_proj, mask_list, self.query_embed.weight, pos[-1], is_train
        )
